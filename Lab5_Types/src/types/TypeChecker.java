...
public class TypeChecker implements CommandVisitor {
    ...
    private void functionReturnError(FunctionDefinition node) {
        ...
        String func = "Function " + currentFunctionName + " returns " + currentReturnType + " not " + retType + ".";
    ...
    private void checkArguments(FuncType func, FunctionDefinition node) {
        ...
        for(Type t : types) {
            if (vt.equivalent(t)) {
                put(node, new ErrorType("Function " + func_name + " has a void argument in position " + pos + "."));
            }
            if (t instanceof ErrorType) {
                et = (ErrorType) t;
                put(node, new ErrorType("Function " + func_name + " has an error in argument in position " + pos + ": " + et.getMessage()));
            }
            pos++;
        }
    }
